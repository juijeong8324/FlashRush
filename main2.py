...
def main():
    ...
    parser.add_argument('--celeba_crop_size', type=int,
                        default=178, help='crop size for the CelebA dataset')
    parser.add_argument('--image_size', type=int,
                        default=256, help='image resolution')
    ...
    num_gpus = torch.cuda.device_count()  # 2 GPUs
    gpu_id = rank % num_gpus  # Map rank to GPU ID
    torch.cuda.set_device(gpu_id)  # Set GPU
    ...
    # starganconfig.
    t = time.time()
    G = Generator(config.g_conv_dim, config.c_dim,
                  config.g_repeat_num).to(gpu_id)  # Allocate GPU
    D = Discriminator(config.image_size, config.d_conv_dim,
                      config.c_dim, config.d_repeat_num).to(gpu_id)  # Allocate GPU
    G.cuda()
    D.cuda()
    # load weights
    print('Loading the trained models from step {}...'.format(config.resume_iters))
    G_path = os.path.join(config.model_save_dir,
                          '{}-G.ckpt'.format(config.resume_iters))
    D_path = os.path.join(config.model_save_dir,
                          '{}-D.ckpt'.format(config.resume_iters))
    ...
    print(f"Rank {rank} using GPU {gpu_id}: Model loaded successfully.")
    if rank == 0:
    ...
    for i, (x_real, c_org) in enumerate(celeba_loader):
        ...
        if i % size != rank:
            continue  # This rank skips this task

        print(f"Rank {rank} using GPU {gpu_id}: Image {i}")
        x_real = x_real.cuda(gpu_id)
        c_trg_list = create_labels(
            c_org, config.c_dim, 'CelebA', config.selected_attrs)

        x_fake_list = [x_real]

        # generate adv in lab space
        t = time.time()
        x_adv, pert = lab_attack2(
            x_real, c_trg_list, gpu_id, G, iter=config.attack_iters)
    ...
    # Merge Metrics
    t_mpi = time.time()
    total_l2_error = comm.reduce(
        l2_error.detach().cpu().numpy(), op=MPI.SUM, root=0)
    total_ssim = comm.reduce(ssim, op=MPI.SUM, root=0)
    total_psnr = comm.reduce(psnr, op=MPI.SUM, root=0)
    total_n_samples = comm.reduce(n_samples, op=MPI.SUM, root=0)
    total_n_dist = comm.reduce(n_dist, op=MPI.SUM, root=0)

    print(f"************MPI Communication rank{rank}: {time.time() - t_mpi}")
    if rank == 0:
        # Print metrics
        print('{} images.L2 error: {}. ssim: {}. psnr: {}. n_dist: {}'.format(total_n_samples,
                                                                              total_l2_error / total_n_samples,
                                                                              total_ssim / total_n_samples,
                                                                              total_psnr / total_n_samples,
                                                                              float(total_n_dist) / total_n_samples))
...
if __name__ == '__main__':
    ...
    print(f"************main {rank}: {time.time() - start_time}")
