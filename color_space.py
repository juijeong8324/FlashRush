...
def rgb2xyz(rgb):  # rgb from [0,1]
    ...
    mask = (rgb > .04045).type(torch.FloatTensor).to(rgb.device)
    if (rgb.is_cuda):
        mask = mask.cuda()

    rgb = (((rgb + .055) / 1.055) ** 2.4) * mask + rgb / 12.92 * (1 - mask)

    x = .412453 * rgb[:, 0, :, :] + .357580 * \
        rgb[:, 1, :, :] + .180423 * rgb[:, 2, :, :]
    y = .212671 * rgb[:, 0, :, :] + .715160 * \
        rgb[:, 1, :, :] + .072169 * rgb[:, 2, :, :]
    z = .019334 * rgb[:, 0, :, :] + .119193 * \
        rgb[:, 1, :, :] + .950227 * rgb[:, 2, :, :]
    out = torch.cat(
...
def xyz2rgb(xyz):
    ...
    mask = (rgb > .0031308).type(torch.FloatTensor).to(rgb.device)
    if (rgb.is_cuda):
        mask = mask.cuda()

    rgb = (1.055 * (rgb ** (1. / 2.4)) - 0.055) * \
        mask + 12.92 * rgb * (1 - mask)
...
def xyz2lab(xyz):
    ...
    sc = torch.Tensor((0.95047, 1., 1.08883))[
        None, :, None, None].to(xyz.device)
    if (xyz.is_cuda):
        sc = sc.cuda()

    xyz_scale = xyz / sc

    mask = (xyz_scale > .008856).type(torch.FloatTensor).to(xyz.device)
    if (xyz_scale.is_cuda):
        mask = mask.cuda()

    xyz_int = xyz_scale ** (1 / 3.) * mask + \
        (7.787 * xyz_scale + 16. / 116.) * (1 - mask)
...
def lab2xyz(lab):
    ...
    mask = (out > .2068966).type(torch.FloatTensor).to(out.device)
    if (out.is_cuda):
        mask = mask.cuda()
